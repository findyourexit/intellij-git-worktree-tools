...
Use the [Bug Report](https://github.com/findyourexit/intellij-git-arborist/issues/new?template=bug_report.yml) issue template. Include the IDE version, plugin version (from **Settings > Plugins**), and the full stack trace if one appears in the IDE log (**Help > Show Log in…**).

─

## Proposing features

Open a [Feature Request](https://github.com/findyourexit/intellij-git-arborist/issues/new?template=feature_request.yml) before starting significant work, so the approach can be discussed first.
